...
import { OblioApi } from './oblio-api';
...
describe('OblioApi', () => {
  let oblioApi: OblioApi;
  const testEmail = 'test@example.com';
  const testSecret = 'secret123';
  const testCif = 'RO123456';

  beforeEach(() => {
    oblioApi = new OblioApi(testEmail, testSecret, testCif);
  });
  ...
  describe('_generateAccessToken', () => {
    ...
    it('should throw error when authorization fails', async () => {
      ...
      await expect(oblioApi._generateAccessToken()).rejects.toEqual(
        new OblioApiException(
          `Error authorize token! HTTP status: ${mockError.status}`,
          mockError.status,
        ),
      );
    });
  ...
  describe('createInvoice', () => {
    ...
    it('should throw error when invoice creation fails', async () => {
      ...
      // Mock the buildRequest method to simulate error
      const mockResponse = {
        status: 400,
      ...
      await expect(oblioApi.createInvoice(mockInvoiceData)).rejects.toEqual(
        new OblioApiException(mockResponse.data.statusMessage, mockResponse.status),
      );
    });
  });
  ...
  describe('createProforma', () => {
    ...
    it('should throw error when proforma creation fails', async () => {
      ...
      await expect(oblioApi.createProforma(mockProformaData)).rejects.toEqual(
        new OblioApiException(mockResponse.data.statusMessage, mockResponse.status),
      );
    });
  });
  ...
  describe('createNotice', () => {
    ...
    it('should throw error when notice creation fails', async () => {
      ...
      await expect(oblioApi.createNotice(mockNoticeData)).rejects.toEqual(
        new OblioApiException(mockResponse.data.statusMessage, mockResponse.status),
      );
    });
  });
  ...
  describe('createEInvoice', () => {
    ...
    it('should throw error when e-invoice creation fails', async () => {
      ...
      await expect(oblioApi.createEInvoice(seriesName, number)).rejects.toEqual(
        new OblioApiException(mockResponse.data.statusMessage, mockResponse.status),
      );
    });
  });
  ...
  describe('collect', () => {
    ...
    it('should throw error when collection fails', async () => {
      ...
      await expect(oblioApi.collect(seriesName, number, collectData)).rejects.toEqual(
        new OblioApiException(mockResponse.data.statusMessage, mockResponse.status),
      );
    });
  });
  ...
  describe('nomenclature', () => {
    ...
    it('should throw error for invalid nomenclature type', async () => {
      ...
      await expect(oblioApi.nomenclature(type, name, filters)).rejects.toEqual(
        new OblioApiException('Type not implemented'),
      );
    });
  ...
  describe('_checkType', () => {
    ...
    it('should throw error for invalid document type', () => {
      expect(() => oblioApi._checkType('invalid')).toThrow(OblioApiException);
    });
  });
  ...
  describe('createDoc', () => {
    ...
    it('should throw error for invalid document type', async () => {
      ...
      await expect(oblioApi.createDoc(docType, docData)).rejects.toEqual(
        new OblioApiException('Type not supported', 0),
      );
    });
  ...
  describe('get', () => {
    ...
    it('should throw error for invalid document type', async () => {
      await expect(oblioApi.get('invalid', 'TEST', 123)).rejects.toEqual(
        new OblioApiException('Type not supported', 0),
      );
    });
    ...
    it('should handle not found error', async () => {
      ...
      await expect(oblioApi.get('invoice', 'TEST', 123)).rejects.toEqual(
        new OblioApiException(mockResponse.data.statusMessage, mockResponse.status),
      );
    });
  });
  ...
  describe('getAccessToken', () => {
    it('should return existing valid token', async () => {
      const validToken = new AccessToken({
    ...
    it('should generate new token when current token is expired', async () => {
      ...
      (oblioApi as any).accessToken = expiredToken;
      jest.spyOn(oblioApi as any, '_generateAccessToken').mockResolvedValueOnce(validToken);

      const token = await (oblioApi as any).getAccessToken();
      expect(token.access_token).toBe('new-token');
    });
  });
  ...
  describe('_checkType', () => {
    it('should accept valid document types', () => {
      ...
      validTypes.forEach((type) => {
        expect(() => {
          (oblioApi as any)._checkType(type);
        }).not.toThrow();
      });
    });
    ...
    it('should throw error for invalid document type', () => {
      expect(() => {
        (oblioApi as any)._checkType('invalid');
      }).toThrow(OblioApiException);
    });

    it('should throw error for empty document type', () => {
      expect(() => {
        (oblioApi as any)._checkType('');
      }).toThrow(OblioApiException);
    });

    it('should throw error for undefined document type', () => {
      expect(() => {
        (oblioApi as any)._checkType(undefined);
      }).toThrow(OblioApiException);
    });
  });
});
